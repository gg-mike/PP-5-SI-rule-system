...
class TypefaceApp implements ActionListener {
	...
	ResourceBundle typeFaceResources;
	...
	TypefaceApp() {
		try {
			typeFaceResources = ResourceBundle.getBundle("resources.TypefaceResources", Locale.getDefault());
		...
		JFrame jfrm = new JFrame(typeFaceResources.getString("TypefaceApp"));
		...
		prevButton = new JButton(typeFaceResources.getString("Prev"));
		prevButton.setActionCommand("Prev");
		buttonPanel.add(prevButton);
		prevButton.addActionListener(this);

		nextButton = new JButton(typeFaceResources.getString("Next"));
		...
		/* ======================== */
		/* Load the type face program. */
		/* ======================== */

		clips = new Environment();

		clips.load("typefaceapp.clp");
	...
	private void nextUIState() throws Exception {
		...
		if (fv.getFactSlot("state").toString().equals("final")) {
			...
			nextButton.setText(typeFaceResources.getString("Restart"));
			prevButton.setVisible(true);
		} else if (fv.getFactSlot("state").toString().equals("initial")) {
			nextButton.setActionCommand("Next");
			nextButton.setText(typeFaceResources.getString("Next"));
		...
		for (int i = 0; i < pv.size(); i++) {
			...
			if (bv.toString().equals(selected))
				rButton = new JRadioButton(typeFaceResources.getString(bv.toString()), true);
			else
				rButton = new JRadioButton(typeFaceResources.getString(bv.toString()), false);
		...
		String theText = typeFaceResources.getString(fv.getFactSlot("display").symbolValue());
